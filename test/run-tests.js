...
const tests = [
  ...
  ["job packet showcase turns packet pieces into sayable lines", testJobPacketShowcaseCard],
  ["interview readiness dock shows sayable job material", testInterviewReadinessDockCard],
...
function testBeginnerMissionDockCard() {
  ...
  const progress = createInitialProgress(now);
  let card = beginnerMissionDockCard(progress, now);

  assert.equal(card.title, "Mission Dock");
  assert.equal(card.activeId, "start");
  assert.equal(card.action.kind, "profile");
  assert.equal(card.lanes.length, 5);
  assert.deepEqual(card.lanes.map((lane) => lane.id), ["start", "daily", "role", "packet", "review"]);
  assert.equal(card.lanes.find((lane) => lane.id === "start").status, "active");
  assert.ok(card.headline.includes("next playable move"));
  assert.ok(card.progressLabel.includes("landing"));
  assert.ok(card.progressLabel.includes("packet"));
  assert.ok(card.reassurance.includes("one tiny action"));
  assert.doesNotMatch(JSON.stringify(card), /repo|project implementation|build a project|coding task/i);

  selectLearnerProfile(progress, "beginner");
  card = beginnerMissionDockCard(progress, now);
  assert.equal(card.activeId, "start");
  assert.equal(card.action.kind, "recommend");
  assert.ok(card.nextMove.includes("single-choice"));

  const question = flattenQuestions().find((item) => item.type === "single");
  answerQuestion(progress, question, question.answer, now);
  card = beginnerMissionDockCard(progress, now);
  assert.equal(card.activeId, "packet");
  assert.equal(card.lanes.find((lane) => lane.id === "daily").status, "done");
  assert.equal(card.lanes.find((lane) => lane.id === "role").status, "started");
  assert.equal(card.lanes.find((lane) => lane.id === "packet").status, "active");
}
...
function testAnswerLootCard() {
  const progress = createInitialProgress(1000);
  const [single, nextSingle] = flattenQuestions().filter((item) => item.type === "single");

  let result = gradeQuestion(single, single.answer);
  answerQuestion(progress, single, single.answer, 1000);
  let card = answerLootCard(single, result, progress);
  assert.equal(card.title, "Answer Loot");
  assert.equal(card.status, "proof");
  assert.ok(card.headline.includes("Loot gained"));
  assert.ok(card.subhead.includes("Recognize"));
  assert.deepEqual(card.badges.map((badge) => badge.id), ["xp", "signal", "loop"]);
  assert.equal(card.badges.find((badge) => badge.id === "xp").value, "+10");
  assert.equal(card.badges.find((badge) => badge.id === "loop").value, "Recall");
  assert.ok(card.nextAction.includes("next tiny prompt"));
  assert.doesNotMatch(JSON.stringify(card), /repo|project implementation|build a project|coding task/i);

  result = gradeQuestion(nextSingle, 99);
  answerQuestion(progress, nextSingle, 99, 2000);
  card = answerLootCard(nextSingle, result, progress);
  assert.equal(card.status, "repair");
...
function testDailyPhraseBankCard() {
  ...
  assert.equal(card.achievementLine, "");
  assert.ok(card.achievementUse.includes("Unlock"));
  ...
  assert.equal(card.achievementTitle, "Role Explorer");
  assert.ok(card.achievementLine.startsWith("I can "));
  assert.ok(card.achievementUse.includes("interview sentence"));
...
function testPitchUnlockPreviewCard() {
  const progress = createInitialProgress(1000);
  const chapter = course.chapters[0];
  let card = pitchUnlockPreviewCard(progress);
  assert.equal(card.title, "Pitch Unlock Preview");
  assert.equal(card.status, "locked");
  assert.equal(card.chapterId, chapter.id);
  assert.equal(card.lines.length, 3);
  assert.deepEqual(card.lines.map((line) => line.id), ["problem", "role", "tradeoff"]);
  assert.ok(card.prompt.includes("agentic workflow"));
  ...
  completeBossQuiz(progress, chapter.id, 8, 8, 1000);
  card = pitchUnlockPreviewCard(progress);
  assert.equal(card.status, "drafting");
  assert.equal(card.nextAction, "Practice interview prompt");
  assert.ok(card.checks.every((check) => check.done === false));
...
function testJobPacketShowcaseCard() {
  const now = 1000;
  const progress = createInitialProgress(now);
  const chapter = course.chapters[0];
  let card = jobPacketShowcaseCard(progress, now);

  assert.equal(card.title, "Packet Showcase");
  assert.equal(card.status, "starter");
  assert.ok(card.headline.includes("0/4"));
  assert.deepEqual(card.lines.map((line) => line.id), ["identity", "evidence", "latest"]);
  assert.ok(card.lines.find((line) => line.id === "identity").text.includes("tiny judgment drills"));
  assert.ok(card.lines.find((line) => line.id === "latest").text.includes("low-friction prompt"));
  assert.ok(card.promise.includes("No separate artifact"));
  assert.doesNotMatch(JSON.stringify(card), /repo|project implementation|build a project|coding task/i);

  const question = flattenQuestions().find((item) => item.type === "single");
  answerQuestion(progress, question, question.answer, now);
  card = jobPacketShowcaseCard(progress, now);
  assert.equal(card.status, "building");
  assert.ok(card.lines.find((line) => line.id === "latest").text.startsWith("I can "));
  ...
  card = jobPacketShowcaseCard(progress, now);
  assert.equal(card.status, "ready");
  assert.ok(card.headline.includes("Three lines"));
  assert.ok(card.rehearsal.includes("Read the three lines"));
  assert.ok(card.lines.find((line) => line.id === "evidence").text.includes("agentic workflow"));
}

function testInterviewReadinessDockCard() {
  const now = 1000;
  const progress = createInitialProgress(now);
  const chapter = course.chapters[0];
  let card = interviewReadinessDockCard(progress, now);

  assert.equal(card.title, "Interview Readiness Dock");
  assert.equal(card.status, "starter");
  assert.equal(card.lanes.length, 4);
  assert.deepEqual(card.lanes.map((lane) => lane.id), ["one-line", "receipt", "evidence", "pitch"]);
  assert.ok(card.headline.includes("sayable pieces"));
  assert.ok(card.progressLabel.includes("packet"));
  assert.ok(card.progressLabel.includes("speaking"));
  assert.ok(card.promise.includes("No blank-page"));
  assert.doesNotMatch(JSON.stringify(card), /repo|project implementation|build a project|coding task/i);

  const question = flattenQuestions().find((item) => item.type === "single");
  answerQuestion(progress, question, question.answer, now);
  card = interviewReadinessDockCard(progress, now);
  assert.equal(card.status, "building");
  assert.equal(card.lanes.find((lane) => lane.id === "one-line").done, true);
  assert.equal(card.lanes.find((lane) => lane.id === "receipt").done, true);
  assert.equal(card.lanes.find((lane) => lane.id === "evidence").done, false);
  ...
  completeBossQuiz(progress, chapter.id, 8, 8, now);
  card = interviewReadinessDockCard(progress, now);
  assert.equal(card.lanes.find((lane) => lane.id === "evidence").done, true);
  assert.equal(card.lanes.find((lane) => lane.id === "pitch").done, false);
  ...
  assert.equal(card.status, "ready");
  assert.equal(card.action.kind, "pitch");
  assert.equal(card.lanes.find((lane) => lane.id === "pitch").done, true);
}
